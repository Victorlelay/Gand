...
dataf <- dataf %>% mutate(other_crit = case_when(
...
dataf <- dataf %>% filter(sexee == "Féminin") 
...
mod_4class <- poLCA(f, data = dataf, nclass = 4, nrep = 10, maxiter = 5000)
# Nombre de classes
K <- 4 # nombre de classes choisies
#############################################################################
...
classification_matrix4 <- matrix(0, nrow = K, ncol = K)

for (true_class in 1:K) {
  idx <- which(assigned_class == true_class)
  if (length(idx) > 0) {
    classification_matrix4[true_class, ] <- colMeans(post[idx, , drop = FALSE])
  }
}

rownames(classification_matrix4) <- paste("Classe assignée", 1:K)
colnames(classification_matrix4) <- paste("Probabilité classe", 1:K)

print(classification_matrix4)

# Taille
# Ajouter la classe latente à chaque individu
data_female_4 <- dataf
data_female_4$classe_latente <- mod_4class$predclass

# Afficher la taille de chaque classe latente
table(data_female_4$classe_latente)
write_dta(data_female_4, "data_female_4.dta")
...
write_dta(data_male_4, "data_male_4.dta")
#############################################################################
